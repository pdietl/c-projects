...
int readInt(FILE * fp)
{
    ...
    if (result == 0) {
        fprintf(stderr,
                "SCAN ERROR: attempt to read an integer failed\n");
...
double readReal(FILE * fp)
{
    ...
    if (result == 0) {
        fprintf(stderr,
                "SCAN ERROR: attempt to read a real number failed\n");
        fprintf(stderr, "offending character was <%c>\n", fgetc(fp));
        exit(2);
    }
    return x;
...
char readChar(FILE * fp)
{
    ...
    if (result == 0) {
        fprintf(stderr,
                "SCAN ERROR: attempt to read a non-whitespace character failed\n");
        fprintf(stderr, "offending character was <%c>\n", fgetc(fp));
        exit(2);
    }
    return x;
...
char readRawChar(FILE * fp)
{
    ...
    if (result == 0) {
        fprintf(stderr,
                "SCAN ERROR: attempt to read a raw character failed\n");
        fprintf(stderr, "offending character was <%c>\n", fgetc(fp));
        exit(2);
    }
    return x;
...
char *readString(FILE * fp)
{
    ...
    if (ch != '\"') {
        fprintf(stderr,
                "SCAN ERROR: attempt to read a string failed\n");
...
void *allocateMsg(size_t size, char *where)
{
    ...
    if (s == 0) {
        fprintf(stderr,
                "%s: could not allocate string, out of memory\n",
                where);
        exit(3);
    }
...
static void *reallocateMsg(void *s, size_t size, char *where)
{
    ...
    if (t == 0) {
        fprintf(stderr,
                "%s: could not reallocate string, out of memory\n",
                where);
        exit(3);
    }
